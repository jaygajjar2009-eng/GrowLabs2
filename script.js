...
const reels = [
  {
    ...
    caption: 'Experience signature flavors and evening ambience in the heart of Bhavnagar.',
...
function renderCategories() {
  ...
  categoryGrid.querySelectorAll('.category-card').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      searchInput.value = val || category;
      renderDirectory();
    });
  });
}
...
function setupFilters() {
  ...
  document.getElementById('openNowToggle').addEventListener('change', (event) => {
    state.filters.openNow = event.target.checked;
    renderDirectory();
  });

  document.getElementById('reelsToggle').addEventListener('change', (event) => {
    state.filters.hasReels = event.target.checked;
    renderDirectory();
  });

  document.getElementById('featuredToggle').addEventListener('change', (event) => {
    state.filters.featured = event.target.checked;
    renderDirectory();
  });

  document.getElementById('newToggle').addEventListener('change', (event) => {
